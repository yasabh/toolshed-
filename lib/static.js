...
// this the vendored licence falls to application/octet-stream, which a browser
// reads as "save me" — so the one link whose whole job is to *show* the licence
// downloads a file called LICENSE instead. Listed rather than defaulting every
// extensionless file to text: public/ holds only what we put there, but the next
// person should not have to know that for the rule to be safe.
const NAMED = new Map([
  ["license", "text/plain; charset=utf-8"],
  ["copying", "text/plain; charset=utf-8"],
  ["notice", "text/plain; charset=utf-8"],
]);

const contentType = (file) =>
  TYPES[extname(file)] ||
  NAMED.get(basename(file).toLowerCase()) ||
